...
public:
	...
	inline CMemory Relative( const ptrdiff_t PreOffset = 0x0, const ptrdiff_t PostOffset = 0x0 ) const
	{
		auto hBegin = m_ptr + PreOffset;
		hBegin += *reinterpret_cast< int32_t* >( hBegin ) + 0x4;
		hBegin += PostOffset;
		return { hBegin };
	}
